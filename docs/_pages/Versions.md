...
### Fixed
- Logging "Log Queue Messages" switch was inversed so when it was on, it was actually off and vice versa.
- Issue with parsing comments in scripts if comment contained a "/" character
- Script output labels not shown in the Flow
- Editor would show "Pending Changes" prompt if a code editor was previously opened.  Eg edit a script, then go to Files and view a file.  Closing that side window would cause the prompt to appear
...
- Docker Nodes now have correct mapping of FFMPEG when registered

### Updates
- Documenation now points to new documenation site https://docs.fileflows.com
